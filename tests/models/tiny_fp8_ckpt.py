    ...
    *,
    ...
    cfg = {
        ...
        "quantization_config": {
            ...
            # the bf16 Linear leaves (the reader reads every Linear under the config's scheme)
            "modules_to_not_convert": [
                "lm_head",
                "model.language_model.embed_tokens",
                *[f"model.language_model.layers.{li}.linear_attn.in_proj_{p}"
                  for li in range(layers) if li % 4 != 3 for p in ("b", "a")],
                *[f"model.language_model.layers.{li}.mlp.{n}"
                  for li in range(layers) for n in ("gate", "shared_expert_gate")],
            ],
        },
    }
...
def make_tiny_mixed_ckpt(
    ...
    *,
    ...
    Per-tensor FP8 attention/GDN projections (weight + scalar weight_scale + input_scale)
    + NVFP4 shared_expert, and a BF16 lm_head: the case where the model holds a
    vocab-sharded ParallelLMHead while the dense reader must shard lm_head like
    embed_tokens. ``routed_experts`` adds the per-expert NVFP4 tensors (the resident
    fused-backend path); without them the experts are absent (offload cache).
    quantized_layers lists every quantized module -- the #438 reader is config-driven."""
    from safetensors.torch import save_file
    ...
    for li in range(layers):
        ...
        if li % 4 != 3:  # GDN (linear attention)
            ...
            for p, rows in (("in_proj_qkv", QKV), ("in_proj_z", Z), ("out_proj", O)):
                ...
                t[f"{g}.{p}.input_scale"] = scale1()
        else:  # full attention
            a = f"{pre}.self_attn"
            for p, rows in (("q", Q), ("k", KV), ("v", KV)):
                t[f"{a}.{p}_proj.weight"] = fp8((rows, H))
                t[f"{a}.{p}_proj.weight_scale"] = scale1()
                t[f"{a}.{p}_proj.input_scale"] = scale1()
            t[f"{a}.o_proj.weight"] = fp8((O, ODIM))
            t[f"{a}.o_proj.weight_scale"] = scale1()
            t[f"{a}.o_proj.input_scale"] = scale1()
            t[f"{a}.q_norm.weight"] = bf16((HEAD_DIM,))
            t[f"{a}.k_norm.weight"] = bf16((HEAD_DIM,))
    ...
    cfg = {
        ...
        "quantization_config": {
            ...
            # the new reader is config-driven: every quantized module must be listed (or
            # its container, which the ancestor match covers)
            "quantized_layers": {
                **{f"model.language_model.layers.{li}.self_attn.{p}_proj": {"quant_algo": "FP8"}
                   for li in range(layers) if li % 4 == 3 for p in ("q", "k", "v", "o")},
                **{f"model.language_model.layers.{li}.linear_attn.{p}": {"quant_algo": "FP8"}
                   for li in range(layers) if li % 4 != 3
                   for p in ("in_proj_qkv", "in_proj_z", "out_proj")},
                **{f"model.language_model.layers.{li}.mlp.shared_expert": {"quant_algo": "W4A16_NVFP4"}
                   for li in range(layers)},
                **({f"model.language_model.layers.{li}.mlp.experts": {"quant_algo": "W4A16_NVFP4"}
                    for li in range(layers)} if routed_experts else {}),
            },
        },
    }
    json.dump(cfg, open(f"{out_dir}/config.json", "w"), indent=1)
