...
``_DenseReader._shard_part`` (the reader's per-part sharding of weight + scale roles),
the end-to-end ``iter_weights`` sharding for both checkpoint kinds, and the
TP-sharded resident fp8 expert banks.
"""
...
def _install(ckpt: str) -> None:
    """Install the checkpoint's QuantConfig process-wide, as EngineConfig does before the reader runs."""
    hf = cached_load_hf_config(ckpt)
    set_quant_config(checkpoint_quant_config(ckpt, hf, get_model_spec(hf.architectures[0])))
...
def _reader(cfg):
    return _DenseReader(None, get_model_spec("Qwen3_5MoeForCausalLM"), cfg)
...
# ======================================================================================
# _DenseReader._shard_part (block-fp8 weight + weight_scale_inv, all part kinds)
# ======================================================================================
def test_shard_part_fp8_column_and_row(cfg):
    _set_tp(0, 2)
    reader = _reader(cfg)
    # column part (q_proj) weight + //128 scale
    w = torch.randn(8192, 2048)
    s = torch.randn(64, 16)
    out = reader._shard_part("model.layers.3.self_attn.qkv_proj", 0,
                             {"weight": w, "weight_scale_inv": s})
    assert out["weight"].shape == (4096, 2048)
    assert out["weight_scale_inv"].shape == (32, 16)
    # row-parallel: input dim
    w = torch.randn(2048, 4096)
    s = torch.randn(16, 32)
    out = reader._shard_part("model.layers.3.self_attn.o_proj", 0,
                             {"weight": w, "weight_scale_inv": s})
    assert out["weight"].shape == (2048, 2048)
    assert out["weight_scale_inv"].shape == (16, 16)
    # vocab-parallel (lm_head)
    w = torch.randn(512, 2048)
    out = reader._shard_part("lm_head", 0, {"weight": w})
    assert out["weight"].shape == (256, 2048)


def test_shard_part_fp8_in_proj_qkv_scale(cfg):
    _set_tp(1, 2)
    reader = _reader(cfg)
    key_dim, value_dim = 16 * 128, 32 * 128
    s = torch.randn((2 * key_dim + value_dim) // 128, 16)
    out = reader._shard_part("model.layers.0.linear_attn.in_proj", 0,
                             {"weight_scale_inv": s})
    assert out["weight_scale_inv"].shape == ((key_dim + value_dim // 2) // 128, 16)
