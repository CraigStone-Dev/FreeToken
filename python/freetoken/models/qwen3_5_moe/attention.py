...
class Qwen3_5Attention(BaseOP):
    ...
    TP: qkv_proj is column-merged (per-part sharded, KV heads replicated when
    num_kv < tp), o_proj is row-parallel (all-reduce inside the layer); the KV
    pool and the attention backend run on the TP-local head counts.
    """
    ...
    def __init__(self, config: ModelConfig, layer_id: int, *, prefix: str = ""):
        ...
        tp = get_tp_info()
        # TP-local head counts for forward (reshape/split); the linears get full sizes
        # and shard per part internally (LinearColParallelMerged / LinearRowParallel).
        ...
        self._qkv_split = [
            self._local_num_q * head_dim * 2,
            self._local_kv_attn_dim,
            self._local_kv_attn_dim,
        ]
        self.qkv_proj = LinearColParallelMerged(
            config.hidden_size,
            [self.num_q * head_dim * 2, self.kv_attn_dim, self.kv_attn_dim],
            has_bias=False,
    ...
    def _project(self, x: torch.Tensor):
        """Returns (q, k, v, gate): q [N, local_num_q, head_dim] post qk-norm+rope,
        k [N, local_kv_attn_dim] post norm+rope, v [N, local_kv_attn_dim],
        gate [N, local_qo_attn_dim]."""
        ...
        q = qg[..., : self.head_dim].contiguous()  # [N, local_num_q, head_dim]
